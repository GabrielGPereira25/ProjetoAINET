<x-layouts::main-content :title="auth()->user()->user_type === 'C' ? __('Upload Custom Image') : __('Create Catalog Image')"
                        :heading="auth()->user()->user_type === 'C' ? __('Upload Custom Image') : __('Create Catalog Image')"
                        :subheading="__('Fill in the details below.')">
    
    <div class="flex w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="my-4 p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg max-w-2xl">
            <form action="{{ route('tshirt_images.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
                @csrf
                ...
                <div class="flex flex-col gap-2">
                    <flux:input type="file" name="image_file" :label="__('Image File')" accept="image/png, image/jpeg, image/jpg, image/svg+xml, image/webp" required />
                    <p class="text-xs text-zinc-500">Max size: 4MB. Formats: JPG, PNG, SVG, WEBP.</p>
                </div>
