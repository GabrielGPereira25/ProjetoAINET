<x-layouts::main-content :title="$category->name"
        ...
        <div class="max-full">
            <section>
                <form method="POST" action="{{ route('categories.update', ['category' => $category]) }}" enctype="multipart/form-data">
                    ...
                    @include('partials.form-buttons', ['entity' => 'category', 'value' => $category,
    ...
    <form class="hidden" id="form_to_delete_category_image"
        method="POST"
        action="{{ route('categories.image.destroy', ['category' => $category]) }}">
        @csrf
        @method('DELETE')
    </form>
